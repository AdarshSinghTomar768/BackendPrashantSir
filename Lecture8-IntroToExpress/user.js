...
const userRequestHandler = (req, res) => {
  ...
  if (req.url === "/") {
    ...
    req.on("end",() => {
      ...
      fs.writeFile('user.txt',JSON.stringify(bodyObject),error => console.log('Data written successfully')); // it will work as async function
      console.log(bodyObject); //{ username: 'Adarsh Singh Tomar', gender: 'male' }
      //fs.writeFile('user.txt','Adarsh Singh Tomar'); //it make the file name as user.txt and write the Adarsh Singh Tomar in it.
      res.statusCode = 302;
